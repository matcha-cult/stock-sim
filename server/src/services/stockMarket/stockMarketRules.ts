...
/** AI 新闻驱动涨跌下限（百分比），默认 1.5%，可通过 STOCK_MARKET_AI_MIN_ABS_CHANGE_PERCENT 覆盖。AI 输出绝对值小于此值时抬升到下限 + 原小数位。 */
const STOCK_MARKET_AI_MIN_ABS_CHANGE_PERCENT_ENV = parseFloat(process.env.STOCK_MARKET_AI_MIN_ABS_CHANGE_PERCENT ?? '1.5');
export const STOCK_MARKET_AI_MIN_ABS_CHANGE_PERCENT = Number.isFinite(STOCK_MARKET_AI_MIN_ABS_CHANGE_PERCENT_ENV)
  && STOCK_MARKET_AI_MIN_ABS_CHANGE_PERCENT_ENV > 0
  ? STOCK_MARKET_AI_MIN_ABS_CHANGE_PERCENT_ENV
  : 1.5;
